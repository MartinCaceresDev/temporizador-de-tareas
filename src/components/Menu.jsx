...
export const Menu = () => {
  ...
  return (
    <div className={`relative max-h-min flex flex-col md:flex-row md:pb-4 justify-center items-center pb-9 sm:pb-4 gap-x-2`}>
      ...
      <span
        className='font-medium text-center cursor-pointer border rounded-full py-1.5 px-3 bg-fuchsia-700 text-white hover:bg-fuchsia-900 hover:shadow-md hover:shadow-slate-400 active:bg-orange-600 transition-all'
        onClick={toggleCreatingTask}
      >
        <AddIcon />
      </span>
      <span
        className={`cursor-pointer text-base absolute right-4 bottom-2 mt-1 ${speechAlertOn ? 'text-green-600' : 'text-red-600'}`}
        onClick={toggleSpeechAlert}
      >
